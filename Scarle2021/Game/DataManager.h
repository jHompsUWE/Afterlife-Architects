...
public:
    ...
    static std::pair<int*, int*>& GetRES() {return Get().window_res;}
     ...
     */
    void PopulatePointers(float _aspect_ratio, HWND* _main_window, int* window_width, int* window_height,
        GameData* _game_data, DrawData* _draw_data, DrawData2D* _draw_data2D, ID3D11Device1* _d3d_device,
        ID3D11DeviceContext1* _d3d_context, IEffectFactory* _effect_factory)
    {
        aspect_ratio = _aspect_ratio;
        main_window = _main_window;
        window_res.first = window_width;
        window_res.second = window_height;
...
private:
    ...
    std::pair<int*, int*> window_res {nullptr,nullptr};
